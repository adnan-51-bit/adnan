...
const INCIDENTS_URL = "https://werknetz24.de/api/customers?type=master-zentrale-incidents";

export async function fetchWerknetz24Incidents(fetchImpl = fetch) {
  const result = await fetchWerknetz24Read(INCIDENTS_URL, fetchImpl);
  if (result.ok) return { ...result, incidents: result.data.incidents || [] };
  return result;
}
...
// Agenten-Zentrale + Fehlerzentrale (Full-System-Audit Phase 2, 26.09.2026). Lesen (Agentenliste)
// ueber das bestehende WERKNETZ24_STATUS_SECRET; die beiden Aktionen (Systemwaechter-Pruefung
// erneut ausfuehren, Incident nach gruener Pruefung abschliessen) ueber das separate
// WERKNETZ24_WRITE_SECRET - gleiche Trennung wie beim Kalender. Werknetz24 selbst verweigert einen
// Abschluss, solange die letzte echte Pruefung nicht gruen ist (409) - hier wird nichts vorgetaeuscht.
const AGENTEN_URL = "https://werknetz24.de/api/customers?type=master-zentrale-agenten";
const SYSTEMCHECK_URL = "https://werknetz24.de/api/customers?type=master-zentrale-systemcheck";
const INCIDENT_ABSCHLUSS_URL = "https://werknetz24.de/api/customers?type=master-zentrale-incident-abschliessen";

export async function fetchWerknetz24Agenten(fetchImpl = fetch) {
  const result = await fetchWerknetz24Read(AGENTEN_URL, fetchImpl);
  if (result.ok) return { ...result, agenten: result.data.agenten || [] };
  return result;
}

async function postWerknetz24Write(url, body, fetchImpl, timeoutMs = 25000) {
  if (!werknetz24WriteConfigured()) {
    return { configured: false, reason: "WERKNETZ24_WRITE_SECRET nicht gesetzt" };
  }
  const controller = new AbortController();
  ...
  try {
    ...
    const data = await response.json().catch(() => null);
    if (!response.ok || !data?.ok) {
      return { configured: true, ok: false, status: response.status, error: (data && data.error) || `HTTP ${response.status}`, data };
    }
...
// Laengerer Timeout (25 s): die Systemwaechter-Pruefung fragt nacheinander mehrere externe Dienste ab.
export async function runWerknetz24Systemcheck(fetchImpl = fetch) {
  return postWerknetz24Write(SYSTEMCHECK_URL, {}, fetchImpl);
}

export async function closeWerknetz24Incident(incidentId, fetchImpl = fetch) {
  return postWerknetz24Write(INCIDENT_ABSCHLUSS_URL, { incidentId }, fetchImpl, 10000);
}
