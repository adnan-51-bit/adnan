...
// Sortiert24-Produktfelder (26.09.2026): Beschreibung, Bilder (nur https-Adressen, max. 8), Bestand
// (null = unbekannt, z. B. Dropshipping), Lieferzeit (Freitext vom Lieferanten, nie erfunden).
export function pruefeProduktZusatz(input) {
  const out = {};
  if ("beschreibung" in input) { const b = String(input.beschreibung ?? ""); if (b.length > 5000) throw new Error("Beschreibung zu lang (max. 5000 Zeichen)"); out.beschreibung = b; }
  if ("bilder" in input) {
    const l = Array.isArray(input.bilder) ? input.bilder.map(x => String(x).trim()).filter(Boolean) : [];
    if (l.length > 8) throw new Error("Maximal 8 Bilder");
    for (const u of l) { let ok = false; try { ok = new URL(u).protocol === "https:"; } catch {} if (!ok) throw new Error("Bild-Adresse muss mit https:// beginnen: " + u.slice(0, 60)); }
    out.bilder = l;
  }
  if ("bestand" in input) { const v = input.bestand; if (v === null || v === "") out.bestand = null; else if (Number.isInteger(v) && v >= 0) out.bestand = v; else throw new Error("Bestand muss eine ganze Zahl ≥ 0 sein (oder leer = unbekannt)"); }
  if ("lieferzeit" in input) { const v = input.lieferzeit == null ? null : String(input.lieferzeit).trim(); if (v && v.length > 120) throw new Error("Lieferzeit zu lang"); out.lieferzeit = v || null; }
  return out;
}
...
export async function createProduct(input) {
  ...
  const product = {
    ...
    notiz: input.notiz || "",
    beschreibung: "", bilder: [], bestand: null, lieferzeit: null,
    ...pruefeProduktZusatz(input),
...
export async function updateProduct(id, patch) {
  ...
  const clean = { ...Object.fromEntries(Object.entries(patch).filter(([k]) => allowed.includes(k))), ...pruefeProduktZusatz(patch) };
  for (const k of ["einkaufspreis_cent", "versandkosten_cent"]) if (k in clean && clean[k] !== null && !(Number.isInteger(clean[k]) && clean[k] >= 0)) throw new Error(k + " muss eine ganze Zahl ≥ 0 sein (oder leer)");
  if ("verkaufspreis_cent" in clean && !(Number.isInteger(clean.verkaufspreis_cent) && clean.verkaufspreis_cent > 0)) throw new Error("Verkaufspreis muss größer als 0 sein");
  if ("name" in clean && !String(clean.name).trim()) throw new Error("Name darf nicht leer sein");
...
// ============ BESTELL-EREIGNISSE + WEBHOOK-BELEGE (26.09.2026) ============
// Vorher nur im Prozessspeicher (lib/store.js, lib/idempotency.js) - nach jedem Neustart waren
// Ereignisverlauf und Duplikatschutz weg. Jetzt in Supabase (ecommerce_order_events /
// ecommerce_webhook_receipts, Eindeutigkeit per DB-Constraint), Fallback wie ueberall im Speicher.
function ensureMemoryEvents() {
  if (!memory.events) memory.events = new Map();
  if (!memory.eventKeys) memory.eventKeys = new Map();
  if (!memory.webhookKeys) memory.webhookKeys = new Set();
}

// Speichert ein Ereignis genau einmal pro idempotencyKey. Liefert { duplicate, event }; bei einem
// Duplikat ist event der zuerst gespeicherte Eintrag (bzw. null, falls die DB ihn nicht zurueckgibt).
export async function recordOrderEvent({ id, orderId, type, fromStatus, toStatus, payload = {} }, idempotencyKey) {
  if (!idempotencyKey) throw new Error("recordOrderEvent: idempotencyKey fehlt");
  const row = { id, business_id: BUSINESS_ID, order_id: orderId, type, from_status: fromStatus ?? null, to_status: toStatus ?? null, payload, idempotency_key: idempotencyKey, created_at: new Date().toISOString() };
  if (!supabaseConfigured()) {
    ensureMemoryEvents();
    if (memory.eventKeys.has(idempotencyKey)) return { duplicate: true, event: memory.events.get(memory.eventKeys.get(idempotencyKey)) };
    memory.events.set(row.id, row); memory.eventKeys.set(idempotencyKey, row.id);
    return { duplicate: false, event: row };
  }
  const rows = await supabaseRequest("ecommerce_order_events?on_conflict=idempotency_key", {
    method: "POST", headers: { Prefer: "resolution=ignore-duplicates,return=representation" }, body: JSON.stringify(row),
  });
  if (rows && rows[0]) return { duplicate: false, event: rows[0] };
  const existing = await supabaseRequest("ecommerce_order_events?select=*&idempotency_key=eq." + encodeURIComponent(idempotencyKey));
  return { duplicate: true, event: existing?.[0] || null };
}

// Nachtraegliches Setzen von to_status, sobald der Uebergang ausgewertet ist.
export async function finalizeOrderEvent(id, toStatus) {
  if (!supabaseConfigured()) { ensureMemoryEvents(); const e = memory.events.get(id); if (e) e.to_status = toStatus; return e || null; }
  const rows = await supabaseRequest("ecommerce_order_events?id=eq." + encodeURIComponent(id), { method: "PATCH", headers: { Prefer: "return=representation" }, body: JSON.stringify({ to_status: toStatus }) });
  return rows?.[0] || null;
}

export async function listOrderEvents(orderId) {
  if (!supabaseConfigured()) { ensureMemoryEvents(); return [...memory.events.values()].filter(e => !orderId || e.order_id === orderId); }
  return supabaseRequest("ecommerce_order_events?select=*&order=created_at.desc" + (orderId ? "&order_id=eq." + encodeURIComponent(orderId) : ""));
}

// true = neuer Beleg (verarbeiten), false = schon gesehen (Duplikat, nicht erneut verarbeiten).
export async function claimWebhookReceipt(key, provider) {
  if (!key) return true;
  if (!supabaseConfigured()) { ensureMemoryEvents(); if (memory.webhookKeys.has(key)) return false; memory.webhookKeys.add(key); return true; }
  const rows = await supabaseRequest("ecommerce_webhook_receipts?on_conflict=key", {
    method: "POST", headers: { Prefer: "resolution=ignore-duplicates,return=representation" }, body: JSON.stringify({ key, business_id: BUSINESS_ID, provider }),
  });
  return Boolean(rows && rows[0]);
}

export async function orderStats() {
  const [orders, events] = await Promise.all([listOrders(), listOrderEvents()]);
  // Letztes Ereignis fuer die Agenten-Zentrale (echte letzte Aktivitaet der Automation Engine) -
  // nur Typ/Status/Zeitpunkt, keine Kunden- oder Bestellinhalte.
  const letztes = [...events].sort((x, y) => String(y.created_at).localeCompare(String(x.created_at)))[0] || null;
  const letzteBlockade = [...events].filter(e => e.to_status === "blocked").sort((x, y) => String(y.created_at).localeCompare(String(x.created_at)))[0] || null;
  return {
    orders: orders.length, events: events.length, persistence: storageMode(),
    letztes_ereignis: letztes ? { type: letztes.type, from_status: letztes.from_status, to_status: letztes.to_status, created_at: letztes.created_at } : null,
    letzte_blockade: letzteBlockade ? { type: letzteBlockade.type, created_at: letzteBlockade.created_at } : null,
  };
}
...
export function resetEcommerceStoreForTests() {
  ...
  if (memory.events) memory.events.clear();
  if (memory.eventKeys) memory.eventKeys.clear();
  if (memory.webhookKeys) memory.webhookKeys.clear();
  memory.seeded = false;
  seedOnce();
}
