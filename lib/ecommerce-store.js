...
function seedOnce() {
  ...
  for (const s of seedSuppliers) if (!memory.suppliers.has(s.id)) memory.suppliers.set(s.id, { ...s, erstellt_am: jetzt });
  for (const p of seedProducts) if (!memory.products.has(p.id)) memory.products.set(p.id, { ...p, erstellt_am: jetzt, aktualisiert_am: jetzt });
  memory.seeded = true;
}
seedOnce();
...
export async function createCustomer(input) {
  ...
  const customer = { id: newId("cust"), name, email, adresse: input.adresse || null, erstellt_am: new Date().toISOString() };
...
export async function createOrder(input) {
  ...
  const order = {
    id: newId("order"), kunde_id, positionen,
...
export async function createReturn(input) {
  ...
  const eintrag = { id: newId("ret"), bestellung_id, grund, status: "angefragt", erstellt_am: new Date().toISOString() };
