...
export async function listSystems(){
  if(!configured()) return {storage:"memory",systems:[...seed.map(s=>({...s})),...memory.values()].reduce((a,s)=>{a.set(s.id,s);return a},new Map()).values().toArray?.() || Array.from([...seed.map(s=>({...s})),...memory.values()].reduce((a,s)=>{a.set(s.id,s);return a},new Map()).values())};
...
export async function updateSystem(id,patch){
  ...
  const updated={...current,...clean};memory.set(id,updated);return {storage:"memory",system:updated};
}
