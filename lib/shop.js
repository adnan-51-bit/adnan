...
import { getOrderById, updateOrder, recordOrderEvent, finalizeOrderEvent, storageMode } from "./ecommerce-store.js";
...
export function istVerkaufbar(p) {
  const m = margeCent(p);
  return VERKAUFBARE_STATUS.includes(p?.pipeline_status) && p.verkaufspreis_cent > 0 && m !== null && m > 0;
}
...
export function shopStartGate({ env = process.env, products = [], storage = storageMode(), texte = RECHTSTEXTE } = {}) {
...
export function berechneWarenkorb(positionen, products) {
  ...
  return { zeilen, summe_cent: zeilen.reduce((s, z) => s + z.summe_cent, 0) };
}
