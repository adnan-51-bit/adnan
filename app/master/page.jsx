...
function Tasks({tasks,loading,onSave,onCreate}){const [editing,setEditing]=useState(null);const [creating,setCreating]=useState(false);return <><div className="pageTitle"><div><span>WORK QUEUE</span><h2>Aufgaben & Quality Gates</h2></div><button className="primaryLink" onClick={()=>setCreating(true)}>+ Aufgabe</button></div>{loading?<Panel title="Aufgaben"><p>Daten werden geladen…</p></Panel>:<div className="taskTable">{tasks.map(t=><div className="taskRow" key={t.id}><button onClick={()=>onSave({...t,status:t.status==="Erledigt"?"Offen":"Erledigt"})} className={t.status==="Erledigt"?"check done":"check"}>✓</button><div><strong>{t.title}</strong><small>{t.area} · {t.owner||"system"}{t.due_at?" · "+new Date(t.due_at).toLocaleDateString("de-DE"):""}</small></div><span>{t.priority}</span><em>{t.status}</em><button className="editMini" onClick={()=>setEditing(t)}>Bearbeiten</button></div>)}</div>}{editing&&<TaskEditor task={editing} onClose={()=>setEditing(null)} onSave={async t=>{await onSave(t);setEditing(null)}}/>}{creating&&<TaskEditor task={{title:"",area:"",status:"Offen",priority:"Mittel",owner:"system",due_at:""}} onClose={()=>setCreating(false)} onSave={async t=>{await onCreate(t);setCreating(false)}}/>}</>}
function TaskEditor({task,onClose,onSave}){const [form,setForm]=useState(task);const change=(k,v)=>setForm({...form,[k]:v});return <div className="modalBack"><div className="modal"><div className="modalHead"><h3>{task.id?"Aufgabe bearbeiten":"Neue Aufgabe"}</h3><button onClick={onClose}>×</button></div><label>Titel<input value={form.title||""} onChange={e=>change("title",e.target.value)}/></label><label>Bereich<input value={form.area||""} onChange={e=>change("area",e.target.value)}/></label><label>Status<select value={form.status||"Offen"} onChange={e=>change("status",e.target.value)}><option>Offen</option><option>In Arbeit</option><option>Blockiert</option><option>Erledigt</option></select></label><label>Priorität<select value={form.priority||"Mittel"} onChange={e=>change("priority",e.target.value)}><option>Niedrig</option><option>Mittel</option><option>Hoch</option></select></label><label>Verantwortlich<input value={form.owner||"system"} onChange={e=>change("owner",e.target.value)}/></label><label>Fällig am<input type="datetime-local" value={form.due_at?String(form.due_at).slice(0,16):""} onChange={e=>change("due_at",e.target.value?new Date(e.target.value).toISOString():"")}/></label><div className="modalActions"><button onClick={onClose}>Abbrechen</button><button className="primary" onClick={()=>onSave(form)} disabled={!form.title||!form.area}>Speichern</button></div></div></div>}
...
function Finance({entries,loading,onCreate}){const [open,setOpen]=useState(false);const totals=entries.reduce((a,e)=>{if(e.status==="cancelled")return a;const n=Number(e.amount)||0;if(e.kind==="income")a.income+=n;else a.expense+=n;a.net=a.income-a.expense;return a},{income:0,expense:0,net:0});return <><div className="pageTitle"><div><span>FINANCE CONTROL</span><h2>Finanzzentrale</h2></div><button className="primaryLink" onClick={()=>setOpen(true)}>+ Buchung</button></div><div className="kpis"><Kpi label="Einnahmen" value={formatEUR(totals.income)} note="erfasste bestätigte/pending Buchungen"/><Kpi label="Kosten" value={formatEUR(totals.expense)} note="erfasste Kosten"/><Kpi label="Netto" value={formatEUR(totals.net)} note="Einnahmen minus Kosten"/><Kpi label="Buchungen" value={entries.length} note={loading?"Laden…":"Ledger-Einträge"}/></div><Panel title="Finanzbuchungen">{loading?<p>Daten werden geladen…</p>:entries.length===0?<p>Noch keine echten Finanzdaten verbunden. Keine Zahlen erfunden.</p>:entries.map(e=><div className="taskMini" key={e.id}><span>{e.kind==="income"?"Einnahme":"Kosten"}</span><div><strong>{e.description||e.category}</strong><small>{e.category} · {e.source} · {new Date(e.occurred_at).toLocaleDateString("de-DE")}</small></div><b>{e.kind==="income"?"+":"−"} {formatEUR(Number(e.amount))}</b></div>)}</Panel><Panel title="Finanzregeln"><ul><li>Keine erfundenen Einnahmen oder Kosten.</li><li>Stripe/PayPal erst nach sicherer Integration.</li><li>Bankdaten nur nach expliziter Verbindung.</li><li>Jede reale Zahlung muss nachvollziehbar verbucht werden.</li></ul></Panel>{open&&<FinanceEditor onClose={()=>setOpen(false)} onSave={async form=>{await onCreate(form);setOpen(false)}}/>}</>}
function formatEUR(n){return new Intl.NumberFormat("de-DE",{style:"currency",currency:"EUR"}).format(n)}
function FinanceEditor({onClose,onSave}){const [form,setForm]=useState({kind:"expense",amount:"",category:"",description:"",status:"confirmed",source:"manual",occurred_at:new Date().toISOString()});const change=(k,v)=>setForm({...form,[k]:v});return <div className="modalBack"><div className="modal"><div className="modalHead"><h3>Finanzbuchung</h3><button onClick={onClose}>×</button></div><label>Art<select value={form.kind} onChange={e=>change("kind",e.target.value)}><option value="expense">Kosten</option><option value="income">Einnahme</option></select></label><label>Betrag (EUR)<input type="number" min="0" step="0.01" value={form.amount} onChange={e=>change("amount",e.target.value)}/></label><label>Kategorie<input value={form.category} onChange={e=>change("category",e.target.value)} placeholder="z. B. Hosting"/></label><label>Beschreibung<input value={form.description} onChange={e=>change("description",e.target.value)}/></label><label>Status<select value={form.status} onChange={e=>change("status",e.target.value)}><option value="confirmed">Bestätigt</option><option value="pending">Offen</option><option value="cancelled">Storniert</option></select></label><div className="modalActions"><button onClick={onClose}>Abbrechen</button><button className="primary" disabled={!form.amount||!form.category} onClick={()=>onSave(form)}>Speichern</button></div></div></div>}
...
function Alerts({systems,tasks}){const alerts=[...systems.filter(s=>s[1]==="🔴").map(s=>({type:"KRITISCH",title:s[0],text:s[2]})),...tasks.filter(t=>t.status==="Blockiert").map(t=>({type:"BLOCKIERT",title:t.title,text:t.area+" · "+t.priority}))];return <><div className="pageTitle"><div><span>ATTENTION CENTER</span><h2>Warnungen & Handlungsbedarf</h2></div></div>{alerts.length===0?<Panel title="Alles ruhig"><p>Keine kritischen Warnungen erkannt.</p></Panel>:<div className="alertList">{alerts.map((a,i)=><article className="alertCard" key={i}><span>{a.type}</span><strong>{a.title}</strong><p>{a.text}</p></article>)}</div>}<Panel title="Regel"><p>Kritische Zustände werden sichtbar gemacht. Automatische Folgeprozesse dürfen bei kritischen Fehlern nicht still weiterlaufen.</p></Panel></>}
