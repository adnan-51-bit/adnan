  ...
  return <main className="app">
    ...
    <div className="layout">
      <aside className="sidebar">
        {[
          ["overview","◈","Übersicht"],["businesses","▣","Betriebe"],["alerts","⚠","Fehler & Warnungen"],["tasks","✓","Aufgaben"],["systems","◉","Systeme"],["finance","€","Finanzen"],["automation","↻","Automationen"],["audit","▤","Audit-Log"],["settings","⚙","Einstellungen"]
      ...
      <section className="content">{loading && <div className="notice">Master-Daten werden geladen…</div>}
        ...
        {tab==="alerts" && <Alerts systems={systems} tasks={tasks} businesses={businesses}/>}
...
function Overview({businesses,tasks,systems,qualityGate,qgLoading,recentActivity,goTo}){
  ...
  return <>
    ...
    <Panel title="System-Lage" action="Fehler & Warnungen →" onClick={()=>goTo?.("alerts")}><div className="systemGrid">{systems.map(s=><div className="system" key={s.id}><b>{s.status} {s.name}</b><small>{s.note}</small></div>)}</div></Panel>
...
function Businesses({businesses,search,setSearch,editing,setEditing,saveBusiness}){
  ...
  <div className="businessGrid">{businesses.map(b=><article className="businessCard" key={b.id}><div className="cardHead"><div><span>{b.type}</span><h3>{b.name}</h3></div><em>{b.status}</em></div><p>{b.health} Systemstatus · Umsatz: {b.revenue}</p>{b.liveStatus&&<LiveStatus status={b.liveStatus}/>}{b.id==="werknetz24"&&<WerknetzKalender/>}<div className="chips">{b.modules.map(m=><i key={m}>{m}</i>)}</div><div className="cardActions">{b.link!="#"&&<a href={b.link}>Öffnen →</a>}<button onClick={()=>setEditing(b)}>Bearbeiten</button></div></article>)}</div>
  {editing&&<EditBusiness business={editing} onClose={()=>setEditing(null)} onSave={saveBusiness}/>}</>
}

// Erster echter Schreibzugriff der Master-Zentrale auf Werknetz24 (22.09.2026, "Kommandozentrale"-
// Folgeauftrag). Rein ueber die eigene, bereits geschuetzte /api/master/businesses-Route (kein
// direkter Client->Werknetz24-Aufruf, sonst waeren Secrets im Browser sichtbar). Zeigt ehrlich
// "nicht konfiguriert"/Fehler an, solange WERKNETZ24_STATUS_SECRET/WERKNETZ24_WRITE_SECRET nicht
// gesetzt sind - nie erfundene Termine.
function WerknetzKalender(){
  ...
  function load(){
    setLoading(true);
    fetch("/api/master/businesses?werknetz24Kalender=1").then(r=>r.json()).then(d=>setKalender(d.kalender)).catch(()=>setKalender({configured:false,reason:"Abruf fehlgeschlagen"})).finally(()=>setLoading(false));
  }
  useEffect(load,[]);
  ...
  return <div className="werknetzKalender">
    <div className="werknetzKalenderHead"><strong>📅 Kalender</strong><button className="editMini" onClick={()=>setOpen(true)}>+ Termin</button></div>
    {loading?<small>Lädt…</small>:!kalender?.configured?<small>🔵 Noch nicht konfiguriert ({kalender?.reason})</small>:!kalender?.ok?<small>🟡 {kalender.error}</small>:kalender.termine.length===0?<small>Keine anstehenden Termine.</small>:<ul>{kalender.termine.map(t=><li key={t.id}>{t.titel} · {new Date(t.start).toLocaleString("de-DE")}</li>)}</ul>}
    {notice&&<small className="werknetzNotice">{notice}</small>}
...
function KalenderTerminForm({onClose,onSave}){
  ...
  return <div className="modalBack"><div className="modal"><div className="modalHead"><h3>Werknetz24-Termin anlegen</h3><button onClick={onClose}>×</button></div>
...
// Fehler & Warnungen werden ausschließlich aus bereits vorhandenen, echten Daten abgeleitet
// (Systeme mit Status 🔴/🟡 aus /api/master/systems, blockierte Aufgaben aus /api/master/tasks) -
// keine eigene "Fehler"-Datenschicht, kein erfundener Schweregrad/Ursache, die der Code nicht
// tatsächlich kennt. Bewusst zuvor nicht in der Sidebar verlinkt gewesen (toter Code) - jetzt
// erreichbar, da genau das, was die Master-Zentrale-Übersicht bereits weiß, sichtbar gemacht wird.
function Alerts({systems,tasks,businesses}){
  const critical=systems.filter(s=>s.status==="🔴").map(s=>({severity:"🔴",title:s.name,detail:s.note||"—",area:"System",link:"#systems"}));
  const watch=systems.filter(s=>s.status==="🟡").map(s=>({severity:"🟡",title:s.name,detail:s.note||"—",area:"System",link:"#systems"}));
  const blockedTasks=tasks.filter(t=>t.status==="Blockiert").map(t=>({severity:"🔴",title:t.title,detail:t.area+" · "+businessLabel(t.business_id,businesses),area:"Aufgabe",link:"#tasks"}));
  const alerts=[...critical,...blockedTasks,...watch];
  return <>
    <div className="pageTitle"><div><span>ATTENTION CENTER</span><h2>Fehler & Warnungen</h2></div></div>
    {alerts.length===0?<Panel title="Alles ruhig"><p>Keine kritischen Systeme (🔴) oder blockierten Aufgaben erkannt.</p></Panel>:<div className="alertList">{alerts.map((a,i)=><article className="alertCard" key={i}><span>{a.severity} {a.area}</span><strong>{a.title}</strong><p>{a.detail}</p></article>)}</div>}
    <Panel title="Was hier NICHT drin ist (bewusst, keine Fake-Daten)"><p>Nur Systeme (aus der Systeme-Registry) und blockierte Aufgaben fließen hier ein. Es gibt aktuell keine eigene, detailliertere Fehlerzentrale mit Ursache/Maßnahme/Logs pro Einzelfehler (s. <code>docs/MASTER-CONTROL-ARCHITECTURE.md</code>) - das wäre eine neue Datenschicht, noch nicht gebaut.</p></Panel>
    <Panel title="Regel"><p>Kritische Zustände werden sichtbar gemacht. Automatische Folgeprozesse dürfen bei kritischen Fehlern nicht still weiterlaufen.</p></Panel>
  </>
}
