...
export function BereicheZentrale({ systems, businesses, qualityGate, goTo }) {
  ...
  const zeilen = [
    ...
    { bereich: "Stripe (Werknetz24)", l: JA, s: NEIN, st: NEIN, ...w24Status("stripe"), ziel: W("stripe") },
    { bereich: "Stripe (E-Commerce)", l: NEIN, s: NEIN, st: NEIN, ...mStatus("stripe"), ziel: { tab: "integrations" } },
    { bereich: "PayPal (Werknetz24)", l: JA, s: NEIN, st: NEIN, ...w24Status("paypal"), ziel: { tab: "integrations" } },
    ...
    { bereich: "Systemstatus", l: JA, s: "Systemstatus (Secret)", st: JA + " Recheck", status: schlechteste(systems.filter(s => s.source === "auto").map(s => s.status)), check: systems.find(s => s.source === "auto")?.last_checked_at || null, fehler: systems.filter(s => s.source === "auto" && (s.status === "🔴" || s.status === "🟡")).map(s => s.status + " " + s.name + (s.id === "stripe" ? " (E-Commerce)" : "")).join(", ") || null, ziel: { tab: "systems" } },
