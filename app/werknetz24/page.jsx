...
const ADMIN_DEEP_LINKS = [
  ...
  ["einstellungen", "Einstellungen / Google verbinden"],
];
...
function WerknetzKalender() {
  ...
  // Reparaturphase Werknetz24 (26.09.2026): "+ Termin" war immer aktiv, auch wenn der Kalender gar
  // nicht erreichbar ist (seit dem 22.09. Google "invalid_grant") - ein Klick lief garantiert in einen
  // Fehler (Schein-Steuerung). Jetzt nur aktiv, wenn der Kalender in diesem Moment wirklich antwortet.
  const kalenderBereit = !loading && kalender?.configured && kalender?.ok;
  const googleAbgelaufen = kalender?.ok === false && /invalid_grant/i.test(String(kalender?.error || ""));
  return <>
    <div className="pageTitle"><div><span>TERMINE</span><h2>Kalender</h2></div><button className="primaryLink" disabled={!kalenderBereit} title={kalenderBereit ? "" : "Erst möglich, wenn der Kalender erreichbar ist"} onClick={() => setOpen(true)}>+ Termin</button></div>
    {notice && <div className="notice">{notice}<button onClick={() => setNotice("")}>×</button></div>}
    {googleAbgelaufen && <Panel title="🔴 Google-Verbindung ungültig – KONFIGURATION OFFEN">
      <p>Google meldet <code>invalid_grant</code>: Die gespeicherte Freigabe für <strong>werknetz24@gmail.com</strong> wurde widerrufen oder ist abgelaufen. Betrifft Kalender <em>und</em> Gmail. Termine anzeigen und anlegen ist erst nach einer neuen Freigabe möglich.</p>
      <ol><li>Werknetz24-Verwaltung → <strong>Einstellungen</strong> → „Gmail verbinden“ (die Freigabe umfasst auch den Kalender)</li><li>Bei Google mit werknetz24@gmail.com zustimmen</li><li>Das angezeigte Token in Vercel (Projekt werkbot24-landing) als <code>GOOGLE_REFRESH_TOKEN</code> rotieren und Werknetz24 neu deployen</li></ol>
      <a className="primaryLink" href="https://werknetz24.de/admin-zentrale#einstellungen" target="_blank" rel="noreferrer">Einstellungen in Werknetz24 öffnen ↗</a>
    </Panel>}
    <Panel title="Anstehende Termine">
      {loading ? <p>Lädt…</p>
        : !kalender?.configured ? <p>🔵 Noch nicht konfiguriert ({kalender?.reason})</p>
        : !kalender?.ok ? <p>{googleAbgelaufen ? "🔴 Nicht abrufbar (Google-Verbindung ungültig, siehe oben)." : "🟡 " + kalender.error}</p>
...
function Verbindung() {
  return <>
    ...
    {/* Reparaturphase (26.09.2026): vorher pauschal "Kalender ansehen und Termine anlegen ✅",
        obwohl der Kalender seit dem 22.09. nicht erreichbar ist. Jetzt getrennt nach Lesen /
        echter Steuerung, jeweils mit der Bedingung, unter der es tatsächlich funktioniert. */}
    <Panel title="👁 Lesen (mit Admin-Secret der Master-Zentrale)">
      <ul><li>Live-Systemstatus, offene Probleme, Aufgaben, offene Rechnungssumme (aggregiert)</li><li>Aufgaben (interne Notizen) und die letzten 50 Rechnungen – ohne Kundennamen/Positionen</li><li>Systemwächter-Fehler mit aktuellem Prüfzustand (Master → Fehlerzentrale)</li><li>Agenten mit letztem Lauf/Fehler (Master → Agenten-Zentrale)</li><li>Kalender – <strong>nur wenn die Google-Verbindung gültig ist</strong></li></ul>
    </Panel>
    <Panel title="⚙ Tatsächlich steuerbar">
      <ul><li>Systemwächter-Prüfung erneut ausführen (Master → Fehlerzentrale)</li><li>Fehler als behoben abschließen – nur wenn die letzte echte Prüfung grün ist (sonst verweigert Werknetz24)</li><li>Termin anlegen – <strong>nur wenn der Kalender erreichbar ist</strong> (Knopf sonst gesperrt)</li></ul>
      <p>Nicht steuerbar: Start/Stop/Pause der Agenten (sie laufen nicht dauerhaft), Lisa/Famulor (kein <code>FAMULOR_API_KEY</code>), Easybell (keine Schnittstelle).</p>
    </Panel>
