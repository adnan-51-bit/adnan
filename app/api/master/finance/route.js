...
export async function GET(request){const authError=checkAdminSecret(request);if(authError)return NextResponse.json({ok:false,error:authError.error},{status:authError.status});try{const business_id=new URL(request.url).searchParams.get("business_id")||undefined;const entries=await listFinance({business_id});return NextResponse.json({ok:true,storage:financeStorage(),entries})}catch(e){return NextResponse.json({ok:false,error:e.message},{status:500})}}
